function c = contrasts(uniGLM, cname, cvalue, ctype, predname)
...
c.name = strrep(cname,'*', 'X');
c.value = cvalue;
% Run analysis
r.slm = SurfStatT(uniGLM.slm,cvalue); % t-statistics
...
title1=['T-statistic Map of ' ctype ' ' predname '_' c.name ' (df=' num2str(r.slm.df) ')'];
...
title2=['RFT-Corrected Vector of ' ctype ' ' predname '_' c.name ' (df=' num2str(r.slm.df) ')'];
...
if ~isempty(RFT.peak)
    % Output RFT Results
    diary(fullfile(uniGLM.outdir, cdir, [c.name '_RFT_output.txt']))
    term(RFT.clus)
...
title3=['FDR-Corrected Vector of ' ctype ' ' predname '_' c.name ' (df=' num2str(r.slm.df) ')'];
